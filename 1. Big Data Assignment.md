...
-The file was loaded as an RDD, so it can be distributed to a number of workers. I would parallelize and apply the lambda function using map to the dataset (to allow spark to work on any chunk of data within the list) and filter for each separate context. For each context I would then apply a count() function to the number of tweets weight (and mentions) and limit my result to the ten highest (take(10)). 

Dataframes sit on top of RDDs and are supported by Spark 2.x, which allow us to deal with tabular data. We can use SQL to query the columns within the table.
