...
-  I could not find a way to include the column headings in my TWEET_RETWEET and TWEET_MENTIONS outputs. This caused me problems in Part 2 where I had to manually define the column headers 


- Scripts are saved in HDFS (maria_dev/maria_dev) in this location: root/root/user/Uni_Assignment (apologies for the other files, I was unable to delete them) 
...
Before starting Part 2, I manually saved my TWEET_RETWEET and TWEET_MENTIONS output files into my HDFS 'maria_dev' folder here: /user/maria_dev/TWEET_RETWEET as I had permissions issues with my root folder. Once in HDFS, I was able to read the data from Spark. Although I used the .csv format, my file did not contain any headers, so I had to add these manually using their position index. I also filtered out the columns that I wouldn't need for Part 2. 
